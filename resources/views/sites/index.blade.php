...
@section('content')
    ...
    <style>
        ...
        .kpi-grid { display: flex; flex-wrap: wrap; gap: 12px; margin-bottom: 20px; align-items: stretch; }
        ...
        .kpi-card.featured { flex: 2 1 220px; border-color: #fedf89; background: #fffdf5; }
        ...
        @media (max-width: 980px) { .kpi-card { flex: 1 1 calc(50% - 6px); } .kpi-card.featured { flex: 1 1 100%; order: 99; } .dashboard-hero { grid-template-columns: 1fr; } .hero-actions { justify-content: flex-start; } }
        @media (max-width: 768px) {
            .kpi-grid { flex-direction: column; }
            .kpi-card, .kpi-card.featured { flex: none; width: 100%; }
    ...
    @else
        <section class="kpi-grid" aria-label="{{ __('KPI globali') }}">
            ...
            {{-- Risparmio stimato — sempre ultimo, larghezza variabile --}}
            @if(($kpis['uses_orders'] || $kpis['uses_reservations']) && ($kpis['estimated_total_savings'] ?? 0) > 0)
                <article class="kpi-card featured">
                    <span class="kpi-icon amber">
                        <svg class="icon" aria-hidden="true" viewBox="0 0 16 16" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
